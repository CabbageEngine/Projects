...
# Check to verify folder exists, verify file integrity, then save 
    # vehicle profile data to file
...
def collect_vehicle_options():
    ...
    while True:
        # key = input("Vehicle color (e.g., color, year): ").strip()
        vehicle_color = "Vehicle Color" # Key
        value = input(f"Enter '{vehicle_color}': ").strip() # Value
...
def modify_vehicle_profile():
    ...
    for vehicle in vehicles:
        if vehicle.get('plate') == lookup_plate:
            ...
            with open(DATA_FILE, "w", encoding='utf-8', errors="replace") as file:
...
def clear_profiles(filename=DATA_FILE):
    if filename.exists():
        ...
        print("No vehicle profiles file exists to delete.\n")
...
2. View all saved vehicle profile
