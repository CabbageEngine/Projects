...
DATA_FILE = Path("Vehicle_Database/vehicle_data/vehicle_profiles.json")
...
def collect_vehicle_options():
    ...
    while True:
        typeMenu()
        choice = input("\nEnter vehicle type (1-8): ").strip()
        vehicle_type_label = "Vehicle Type"

        if choice in vehicle_types and choice != '8':
            selected_type = vehicle_types[choice]
            print(f"You've selected a {selected_type} as your vehicle type.\n")
            options[vehicle_type_label] = selected_type
            break
        elif choice == '8':
            print("Skipping vehicle type selection.\n")
            break
        else:
    ...
    while True:
        ...
        value = input(f"{color_label}: ").strip()
...
""")
        ...
        if choice == '1':
            ...
            plate = input('Enter license plate (without "-"): ').strip()
            plate = plate.replace("-","")
